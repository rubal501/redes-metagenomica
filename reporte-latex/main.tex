...
\author{Javier Roberto Rubalcava Cortés\\Posgrado en ciencias Matemáticas, UNAM}
\date{2026}
...
\begin{itemize}[leftmargin=*]
    \item Red sana: \texttt{H\_adjcent\_path\_completeNET.csv}
    \item Red enferma: \texttt{DD\_adjcent\_path\_completeNET.csv}
\end{itemize}

Cada matriz tiene dimensiones $619 \times 619$, donde las filas y columnas representan rutas metabólicas identificadas por sus códigos en la base de datos MetaCyc/HUMAnN (i.e., \texttt{PWY-7277}, \texttt{TRIGLSYN-PWY}). Los valores de entrada fueron binarizados: cualquier valor mayor que cero se codificó como 1 (arista presente) y cero como ausente.

\subsection{Construcción de los grafos}

Ambas matrices se alinearon al mismo conjunto de nodos mediante reindexación, completando con ceros las entradas faltantes. Se construyeron grafos no dirigidos con la biblioteca NetworkX \citep{hagberg2008}. El conjunto final compartido consta de 619 nodos.
...
El nivel de significancia adoptado fue $\alpha = 0.05$. Para las pruebas no paramétricas se utilizó la biblioteca SciPy de Python \citep{virtanen2020}.

\subsection{Prueba de permutación}

Para evaluar si la diferencia observada en el grado promedio entre redes podía atribuirse al azar, se realizó una prueba de permutación siguiendo el enfoque descrito por \citet{jardim2019}. En cada iteración, se combinaron los grados de ambas redes en un único conjunto, se mezclaron aleatoriamente, se dividieron en dos grupos del tamaño original y se calculó la diferencia de medias. La proporción de permutaciones en las que esta diferencia excedió la observada se tomó como $p$-valor empírico.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Una representación de la red diferencial, donde las aristas ganadas se colorearon en verde y las perdidas en rojo discontinuo.
...
\begin{table}[ht]
    \centering
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.38\linewidth}cc}
        \toprule
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.95\linewidth]{figures/degree_comparison.svg}
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.6\linewidth]{figures/permutation_test.svg}
...
El análisis diferencial revela una asimetría notable: se ganan 609 aristas pero solo se pierden 270. Las rutas que pierden toda su conectividad en el estado enfermo, como \texttt{TRIGLSYN-PWY}, \texttt{SPHINGOLIPID-SYN-PWY} y \texttt{NADSYN-PWY}, son metabólicamente relevantes: involucran la síntesis de lípidos estructurales y cofactores esenciales \citep{agrawal2024wikipathways}, cuya desaparición podría reflejar cambios en la composición microbiana o en la capacidad funcional del microbioma.

La discrepancia entre los resultados de las pruebas no paramétricas clásicas (no significativas) y la prueba de permutación (significativa) merece atención. Las pruebas KS y Mann-Whitney evalúan la forma y el orden de rangos de las distribuciones completas, que pueden ser similares globalmente incluso cuando la media difiere. La prueba de permutación, al enfocarse en la diferencia de medias, es más sensible a desplazamientos sistemáticos de la conectividad promedio. Esto es coherente con observar distribuciones de forma similar pero con una media mayor en la red enferma, patrón que queda resumido visualmente en la Figura~\ref{fig:degree-comparison}.
...
\begin{itemize}[leftmargin=*]
    \item La red metabólica del estado de enfermedad es significativamente más densa y conectada que la del estado sano, con un incremento del 38.7\% en el número de aristas y un grado promedio 38.7\% mayor.
    \item El análisis diferencial indica que la transición al estado enfermo supone una ganancia neta de conectividad: 609 aristas nuevas frente a 270 perdidas, con 606 conservadas.
    \item Rutas metabólicas de síntesis lipídica (\texttt{TRIGLSYN-PWY}, \texttt{SPHINGOLIPID-SYN-PWY}) y biosíntesis de cofactores (\texttt{NADSYN-PWY}) \citep{agrawal2024wikipathways} pierden toda su conectividad en el estado de enfermedad, lo que puede tener relevancia biológica.
    \item La prueba de permutación ($p = 0.0067$) confirmó que la diferencia de grado promedio observada es estadísticamente significativa, mientras que las pruebas distribucionales no paramétricas no detectaron diferencias significativas en la forma general de las distribuciones de grado.
    \item El enfoque de redes metabólicas diferenciales es una herramienta útil para identificar cambios funcionales del microbioma asociados a estados de salud y enfermedad, y puede complementar análisis taxonómicos y de expresión génica.
\end{itemize}

\section{Código}
El análisis de la red fue realizado en un notebook de jupyter. El código esta disponible en el repositorio \url{google.com}
